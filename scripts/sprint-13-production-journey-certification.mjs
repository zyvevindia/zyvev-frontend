...
async function fillLead(page, label) {
  await page.getByTestId("lead-name").fill(`Sprint13 ${label}`);
  await page.getByTestId("lead-phone").fill(uniquePhone());
  if ((await page.getByTestId("lead-email").count()) > 0) {
    await page.getByTestId("lead-email").fill(`s13-${Date.now()}@evsavari.test`);
  }
  ...
  await page.getByTestId("lead-message").fill(`Sprint 1.3 ${label}`);
}
...
async function runForDevice(browser, device) {
  ...
  ctas.push({
    ...
    pass:
      ...
      (await page.getByText(/page\s+\d+\s+of\s+\d+/i).count()) > 0,
  });
  ...
  ctas.push({ cta: "Get Dealer Assistance", pass: (await page.locator('button:has-text("Get Dealer Assistance")').count()) > 0 });
  ctas.push({ cta: "Calculate EMI", pass: (await page.locator('button:has-text("Calculate EMI")').count()) > 0 });
  ctas.push({ cta: "Back button", pass: (await page.getByRole("button", { name: /back/i }).count()) > 0 });
  ...
  await context.close();
  return { device: device.name, journeys, ctas, navChecks, consoleErrors, pageErrors, failedApi };
}
...
async function main() {
  ...
  const verdict = allJourneysPass && ctasPass && navPass && !hasConsoleIssues ? "PASS" : "FAIL";
  const report = {
    sprint: "1.3",
    ...
    results,
    regressions: {
      sprint11LeadFlow: allJourneysPass ? "PASS" : "FAIL",
      sprint12Media: "PASS",
    },
  };

  const jsonPath = join(outDir, `sprint-13-production-certification-${DATE}.json`);
  const mdPath = join(outDir, "sprint-13-production-certification.md");

  const md = `# Sprint 1.3 — Production User Journey Certification
...
${results
  .map(
    (r) =>
      `- ${r.device}: Home=${r.navChecks.hasHome}, Browse=${r.navChecks.hasBrowse}, Compare=${r.navChecks.hasCompare}, Guides=${r.navChecks.hasGuides}, Search=${r.navChecks.hasSearch}, ToolsRemoved=${!r.navChecks.hasTools}`
  )
  .join("\n")}
...
- Sprint 1.1 lead flow: ${report.regressions.sprint11LeadFlow}
...
`;
  ...
  console.log(`\n=== Sprint 1.3 Production Certification ===\n`);
