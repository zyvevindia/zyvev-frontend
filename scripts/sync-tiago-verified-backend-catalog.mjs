#!/usr/bin/env node
/**
 * Phase 2 — sync verified Tiago dossier variants to backend tier-1 file catalog.
 * Source: src/data/catalog/verified/tataTiagoEvVerified.js
...
const TEMPLATE_PATH = join(VARIANTS_DIR, "tata-tiago-ev-xt.json");
...
function patchVariantRecord(template, variant, { legacySlug = null } = {}) {
  ...
  record.media = {
    ...record.media,
    heroImage: TATA_TIAGO_FAMILY_MEDIA.heroImage,
    listingThumbnail: TATA_TIAGO_FAMILY_MEDIA.listingImage,
    compareThumbnail: TATA_TIAGO_FAMILY_MEDIA.compareImage,
    ogImage: TATA_TIAGO_FAMILY_MEDIA.heroImage,
  };
  ...
  if (Array.isArray(variant.featureTags) && variant.featureTags.length) {
    record.featureTags = variant.featureTags;
  }
...
function updateManifest(tiagoSlugs) {
  const manifest = JSON.parse(readFileSync(MANIFEST_PATH, "utf8"));
  const slugs = manifest.slugs || [];
  const withoutTiago = slugs.filter(
    (s) => !String(s).startsWith("tata-tiago-ev")
  );
  manifest.slugs = [...withoutTiago, ...tiagoSlugs];
...
export function syncTiagoVerifiedBackendCatalog() {
  ...
  for (const variant of TATA_TIAGO_VERIFIED_VARIANTS) {
  ...
  for (const [legacySlug, canonicalSlug] of Object.entries(
    TIAGO_DOSSIER_SLUG_ALIASES
  )) {
    const legacySource = TATA_TIAGO_VERIFIED_VARIANTS.find(
  ...
  const manifestSlugs = TATA_TIAGO_VERIFIED_VARIANTS.map((v) => v.slug);
  const manifest = updateManifest(manifestSlugs);

  return {
    variantFilesWritten: written.length,
    manifestVariantCount: manifest.slugs.length,
    tiagoManifestSlugs: manifestSlugs,
    legacyAliases: TIAGO_DOSSIER_SLUG_ALIASES,
  };
}

async function main() {
  console.log("\n=== Sync Tiago verified dossier → backend tier-1 ===\n");
  const result = syncTiagoVerifiedBackendCatalog();
  console.log(`Variant JSON files written: ${result.variantFilesWritten}`);
  console.log(`Tiago slugs in manifest: ${result.tiagoManifestSlugs.length}`);
  console.log("\nDone.\n");
}

if (process.argv[1]?.endsWith("sync-tiago-verified-backend-catalog.mjs")) {
  main().catch((err) => {
    console.error(err);
    process.exit(1);
  });
