#!/usr/bin/env node
/**
 * Photo Replacement Sprint — Batch 5 (Final Cleanup)
 * Usage: node scripts/photo-replacement-batch5.mjs [--skip-build] [--skip-audit]
 */
...
const BATCH5_SLUGS = Object.freeze([
  "mg-zs-ev",
  "tata-tiago-ev",
  "byd-atto-3",
...
const seed = loadPhotoReplacementSeed(root, "photoReplacementBatch5Seed.json");
const report = await runPhotoReplacementBatch({
  root,
  seed,
  slugs: BATCH5_SLUGS,
  batchNumber: 5,
  reportBasename: "photo-replacement-batch5",
});
...
report.filesModified.push(
  "scripts/lib/photoReplacementBatch5Seed.json",
  "scripts/lib/photoReplacementSprint.mjs",
  "scripts/photo-replacement-batch5.mjs",
  "package.json"
);
...
writeFileSync(
  join(docsDir, "photo-replacement-batch5.json"),
  `${JSON.stringify(report, null, 2)}\n`,
  "utf8"
);
writeFileSync(
  join(docsDir, "photo-replacement-batch5.md"),
  `${markdown}\n`,
  "utf8"
);
...
console.log("\nWrote:");
console.log("  docs/media/photo-replacement-batch5.md");
console.log("  docs/media/photo-replacement-batch5.json\n");
