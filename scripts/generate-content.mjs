...
import {
  BRAND_DEFS,
  generateBrandPage,
} from "./content-generators/brandPages.mjs";
import {
  generateAllAgentPages,
  buildAgentBestEvsSlugMap,
  buildAgentCompareSlugs,
} from "./content-generators/agentPages.mjs";
import {
  validateRegistry,
  entryFromSeoPage,
...
for (const brand of BRAND_DEFS) {
  const page = generateBrandPage(brand);
  const rel = `public/seo-data/brands/${brand.slug}.json`;
  register(
    entryFromSeoPage(page.seoPage, {
      id: `brand-${brand.slug}`,
      pageType: "brand",
      path: `/brands/${brand.slug}`,
      h1: `${brand.name} Electric Cars in India`,
      filePath: rel,
    }),
    rel,
    page
...
// —— SEO Agent v1 export (20 score-driven pages) ——
const agentPages = generateAllAgentPages();
for (const row of agentPages) {
  const rel = `public/seo-data/${row.spec.slug}.json`;
  register(
    entryFromSeoPage(row.page.seoPage, {
      id: `agent-${row.spec.id}`,
...
const agentBestEvsMap = buildAgentBestEvsSlugMap(agentPages);
const agentCompareSlugs = buildAgentCompareSlugs(agentPages);
...
writeJson("public/seo-data/content-manifest.json", {
  ...
  counts: {
    ...
    brand: BRAND_DEFS.length,
    seo_agent: agentPages.length,
...
const compareSlugs = [
  ...COMPARE_PAIRS.map(([a, b]) => compareSlugForPair(a, b)),
  ...agentCompareSlugs,
];
...
const brandSlugs = BRAND_DEFS.map((b) => b.slug);
...
export const GENERATED_BRAND_SLUGS = ${JSON.stringify(brandSlugs, null, 2)};
