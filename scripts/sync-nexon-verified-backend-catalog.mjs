#!/usr/bin/env node
/**
 * Phase 2 — sync verified Nexon dossier variants to backend tier-1 file catalog.
 * Source: src/data/catalog/verified/tataNexonEvVerified.js
...
const TEMPLATE_PATH = join(
  VARIANTS_DIR,
  "tata-nexon-ev-creative-plus.json"
);
...
function patchVariantRecord(template, variant, { legacySlug = null } = {}) {
  ...
  record.media = {
    ...
    ogImage: TATA_NEXON_FAMILY_MEDIA.heroImage,
  };
...
function updateManifest(nexonSlugs) {
  const manifest = JSON.parse(readFileSync(MANIFEST_PATH, "utf8"));
  const slugs = manifest.slugs || [];
  const withoutNexon = slugs.filter(
    (s) => !String(s).startsWith("tata-nexon-ev")
  );
  manifest.slugs = [...withoutNexon, ...nexonSlugs];
...
export function syncNexonVerifiedBackendCatalog() {
  ...
  for (const variant of TATA_NEXON_VERIFIED_VARIANTS) {
    const record = patchVariantRecord(template, variant);
    written.push(writeVariantFile(variant.slug, record));
  }

  const legacySlug = "tata-nexon-ev-creative-plus";
  const legacySource = TATA_NEXON_VERIFIED_VARIANTS.find(
    (v) => v.slug === NEXON_DOSSIER_SLUG_ALIASES[legacySlug]
  );
  if (legacySource) {
  ...
  const manifestSlugs = TATA_NEXON_VERIFIED_VARIANTS.map((v) => v.slug);
  const manifest = updateManifest(manifestSlugs);

  return {
    variantFilesWritten: written.length,
    manifestVariantCount: manifest.slugs.length,
    nexonManifestSlugs: manifestSlugs,
    legacyAliasFile: legacySlug,
    legacyAliases: NEXON_DOSSIER_SLUG_ALIASES,
  };
}

async function main() {
  console.log("\n=== Sync Nexon verified dossier → backend tier-1 ===\n");
  const result = syncNexonVerifiedBackendCatalog();
  console.log(`Variant JSON files written: ${result.variantFilesWritten}`);
  console.log(`Nexon slugs in manifest: ${result.nexonManifestSlugs.length}`);
  console.log(`Legacy alias file preserved: ${result.legacyAliasFile}`);
  console.log("\nDone.\n");
}

if (process.argv[1]?.endsWith("sync-nexon-verified-backend-catalog.mjs")) {
  main().catch((err) => {
    console.error(err);
    process.exit(1);
  });
